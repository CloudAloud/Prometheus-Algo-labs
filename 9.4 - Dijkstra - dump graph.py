...
def convert(graph):
    for node in graph.nodes:
        ...
        for link in node.toList:
            toList[link.number] = node.toList[link]
        for predecessor in node.predecessor:
            predecessorList.append(predecessor.number)
...
print 'Cost:', graph.nodes[finish].bestpath, ', Paths:' ,graph.nodes[finish].paths

dump(filename + '.' + str(start) + '.dump', graph)
